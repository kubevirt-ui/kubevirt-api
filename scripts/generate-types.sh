...
generate_crd_types() {
  local crds_path=$1
  local pkg_path=$2
  local fallback_type="${3:-unknown | null}"
  local models_dir="${pkg_path}/models"
  local crdtotypes_args=(-i "${pkg_path}/.openapi.yaml" -o "${models_dir}" --fallbackType "${fallback_type}")

  mkdir -p "${crds_path}" "${models_dir}"
  "${CRDTOAPI}" -i "${crds_path}" -o "${pkg_path}/.openapi.yaml"
  rm -rf "${models_dir}"
  mkdir -p "${models_dir}"
  "${CRDPTYPES}" "${crdtotypes_args[@]}"
...
# Empty object schemas (e.g. desiredState with x-kubernetes-preserve-unknown-fields) use --fallbackType.
generate_crd_types "${NMSTATE_CRDS}" nmstate any
