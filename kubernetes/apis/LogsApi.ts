...
export class LogsApi extends runtime.BaseAPI {
  /**
   */
  async logFileHandlerRaw(
    requestParameters: LogFileHandlerRequest,
    initOverrides?: RequestInit,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters.logpath === null || requestParameters.logpath === undefined) {
      throw new runtime.RequiredError(
        'logpath',
        'Required parameter requestParameters.logpath was null or undefined when calling logFileHandler.',
      );
    }
    ...
    const response = await this.request(
      {
        path: `/logs/{logpath}`.replace(
          `{${'logpath'}}`,
          encodeURIComponent(String(requestParameters.logpath)),
        ),
        method: 'GET',
